...
public class BoundaryConditionsPanel extends AbstractGUIPanel implements IBoundaryConditionsPanel {
    ...
    private Map<BoundaryType, BoundaryTypePanel> panelsByType = new HashMap<>();
    ...
    private SelectBoundaryConditionTypeAction selectBoundaryConditionListener;
    ...
    private void initTypeField() {
        ...
        selectBoundaryConditionListener = new SelectBoundaryConditionTypeAction(patchTypeField);
    ...
    class SelectBoundaryConditionTypeAction implements ActionListener {
        private JComboBox<String> patchTypeField;

        public SelectBoundaryConditionTypeAction(JComboBox<String> patchTypeField) {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (patches.length > 0) {
                for (Patch patch : patches) {
                    fixAMIPatch(patch);
                    patch.setPhysicalType(boundaryType);
            ...
            model.boundaryTypeChanged(boundaryType);
        }

        private void fixAMIPatch(Patch patch) {
            if (patch.getPhysicalType().isCyclicAMI() && patch.getDictionary().found(NEIGHBOUR_PATCH_KEY)) {
                String neighbourPatchName = patch.getDictionary().lookup(NEIGHBOUR_PATCH_KEY);
                for (Patch p : model.getPatches().filterProcBoundary()) {
                    if (p.getName().equals(neighbourPatchName)) {
                        p.resetToDefault();
                    }
                }
            }
    ...
    private void updatePatchTypeField(Patch[] patches) {
        ...
        if (patches.length > 0) {
            ...
            if (patchesAreOfTheSameType(patches)) {
                patchTypeField.setSelectedItem(patches[0].getPhysicalType().isEditable() ? patches[0].getPhysicalType().getKey() : null);
                patchTypeField.setEnabled(patches[0].getPhysicalType().isEditable());
            } else {
                patchTypeField.setEnabled(areEditable(patches));
    ...
    private boolean areEditable(Patch[] patches) {
        for (Patch patch : patches) {
            if (!patch.getPhysicalType().isEditable()) {
                return false;
            }
        }
        return true;
    ...
    private boolean patchesAreOfTheSameType(Patch[] patches) {
        ...
        for (Patch patch : patches) {
            if (type == null)
                type = patch.getPhysicalType();
            else if (patch.getPhysicalType() != type)
                return false;
    ...
    private void activateBoundaryMesh(Patch[] patches) {
        if (patches.length > 0) {
            BoundaryType boundaryType = patchesAreOfTheSameType(patches) ? patches[0].getPhysicalType() : null;
            ...
            if (boundaryType != null) {
                if (boundaryType.hasBoundaryConditions()) {
                    ...
                    panelsByType.get(boundaryType).resetToDefault();
    ...
    public void savePatch(Patch patch) {
        patch.setPhysicalType(activeBoundaryType);
        if (panelsByType.containsKey(activeBoundaryType)) {
            panelsByType.get(activeBoundaryType).saveToPatch(patch);
            ModulesUtil.saveToBoundaryConditions(modules, patch.getBoundaryConditions());
        } else {
