...
public class TimePanel extends ChooserPanel {
    ...
    public TimePanel() {
        ...
        addChoice(Time.STEADY.label());
        addChoice(Time.TRANSIENT.label());
    }

    public void updateFromState(State state) {
        if (state.isSteady())
            select(Time.STEADY.label());
        else if (state.isTransient())
            select(Time.TRANSIENT.label());
        else
            selectNone();
    }
    ...
    public Time getTime() {
        ...
        if (selectedState.equals(Time.STEADY.label()))
            return Time.STEADY;
        else if (selectedState.equals(Time.TRANSIENT.label()))
            return Time.TRANSIENT;
        return Time.NONE;
    }

    public boolean isSteady() {
        return getSelectedState().equals(Time.STEADY.label());
    }

    public boolean isTransient() {
        return getSelectedState().equals(Time.TRANSIENT.label());
    }
