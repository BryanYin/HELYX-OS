...
import eu.engys.gui.solver.postprocessing.panels.actions.ShowCrosshairAction;
...
public class ResidualsPanel extends DefaultGUIPanel implements SelectedViewProvider {
    ...
    private void populateToolbar() {
        titleToolbar.add(showCrosshairButton = UiUtil.createToolBarToggleButton(new ShowCrosshairAction(), true));
    ...
    @Override
    public void load() {
        residualsView.updateConfiguration();
        if (model.getSolverModel().getServerState().getSolverState().isDoingSomething()) {
            // do nothing
        } else {
            residualsView.reset();
        }
        residualsView.handleStateChanged(model.getState());
    }
