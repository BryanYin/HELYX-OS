...
import static eu.engys.core.project.zero.fields.Fields.POINT_MOTION_U;
...
public class CustomFileDialog {
    ...
    private JComboBoxWithItemsSupport<String> typeCombo;
    ...
    private JComboBoxWithItemsSupport<String> createTypeCombo() {
        JComboBoxWithItemsSupport<String> typeCombo = selectFieldWithItemSupport(CustomFileType.keys(), CustomFileType.labels());
    ...
    private JPanel createButtonsPanel() {
        ...
        JButton cancelButton = new JButton(new AbstractAction(DIALOG_CANCEL_LABEL) {
    ...
    private class AddCustomFile extends AbstractAction {
        ...
        private void addFile() {
            ...
            if (type == null) {
                JOptionPane.showMessageDialog(dialog, "Please, specify a type for the file", "Warning", JOptionPane.WARNING_MESSAGE);
                return;
            }
            ...
            if (exists) {
                if(customFile.getType().isDirectory()){
                    //do nothing
                } else {
                    if (isNewFile()) {
                ...
                /*
                 * If a field does not exist I load the empty BCs from an existing one
                 */
                if(customFile.getType().isField() && !model.getFields().isEmpty()){
                    Dictionary dict = new Dictionary(fileName);
                    dict.add(CustomUtils.getCleanBoundaryField(model.getFields().orderedFields().get(0)));
    ...
    public void fixTypeCombo(Object selectedItem) {
        if (selectedItem != null) {
            ...
            case "0":
                ...
                if (DICTIONARY.getKey().equals(typeCombo.getItemAt(typeCombo.getSelectedIndex()))) {
            ...
            default:
                ...
                if (FIELD.getKey().equals(typeCombo.getItemAt(typeCombo.getSelectedIndex()))) {
    ...
    private class ValidFieldFileFilter implements FileFilter {
        ...
        private boolean isDynamic(String name) {
            return name.equals(POINT_MOTION_U) || name.equals(POINT_DISPLACEMENT);
        }
    }
    ...
    private class ValidDirectoryFileFilter implements FileFilter {
        ...
        public boolean accept(File pathname) {
            ...
            boolean isVisible = !pathname.isHidden();
            boolean isNotAlreadyUsed = !children.contains(pathname.getName());
            return isDir && isVisible && isNotAlreadyUsed;
        }
    }
