...
public class ScriptEditor {
	...
	private void initEditor() {
		...
		editor.getDocument().putProperty(DefaultEditorKit.EndOfLineStringProperty, IOUtils.LNX_EOL);
	...
	private class ImportAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (retVal.isApprove()) {
				...
				try {
					defaultFile = File.createTempFile("xxx", null);
					IOUtils.writeLinesToFile(defaultFile, getFileLinesAndOldOnesCommented(chooser.getSelectedFile()));
		...
		private List<String> getFileLinesAndOldOnesCommented(File file) throws IOException {
			List<String> commentedLines = getCurrentFileCommentedLines();

			List<String> newLines = new ArrayList<>();
			newLines.addAll(FileUtils.readLines(file));
			newLines.add("");
